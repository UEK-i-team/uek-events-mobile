...
import { styles, MONTH_ITEM_WIDTH } from "./timeline-scroller.styles";
...
const Day = memo(function Day({setSelectedDate, item, isDaySelected, itemWidth, visibleEventId, isDark}: DayProps) {
...
export function TimelineScroller({events, selectedDate, onDateSelect, visibleEventId}: TimelineScrollerProps) {
    ...
    useEffect(() => {
      if (activeDate) {
        setVisibleMonthDate(prev => {
          if (!prev || prev.getMonth() !== activeDate.getMonth() || prev.getFullYear() !== activeDate.getFullYear()) {
    ...
    const uniqueMonths = useMemo(() => {
      ...
      datesDays.forEach(d => {
        ...
        if (!seen.has(id)) {
          seen.add(id);
          months.push({ id, date: d.date, label: formatMonthPL(d.date) });
        }
      });
      return months;
    }, [datesDays]);
    ...
    useEffect(() => {
      Animated.spring(scrollAnim, {
        toValue: -activeMonthIndex * MONTH_ITEM_WIDTH,
    ...
    return (
      <View style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const isDaySelected = selectedDate ? isSameDay(item.date, selectedDate) : false;
